...
class SoftDisableAlert(Alert):
  def __init__(self, alert_text_2: str):
    super().__init__("운전대를 잡으세요", alert_text_2,
...
class StartupAlert(Alert):
  def __init__(self, alert_text_1: str, alert_text_2: str = "항상 운전대를 잡고 도로를 주시하세요", alert_status=AlertStatus.normal):
    super().__init__(alert_text_1, alert_text_2,
...
EVENTS: dict[int, dict[str, Alert | AlertCallbackType]] = {
  ...
  EventName.startup: {
    ET.PERMANENT: StartupAlert("항상 운전대를 잡고 도로를 주시하세요")
  },
  ...
  EventName.startupNoSecOcKey: {
    ET.PERMANENT: NormalPermanentAlert("암호화로 인해 자동차를 인식할수 없습니다",
  ...
  EventName.pedalPressed: {
    ET.USER_DISABLE: EngagementAlert(AudibleAlert.disengage),
    ET.NO_ENTRY: NoEntryAlert("엑셀레이터 감지",
